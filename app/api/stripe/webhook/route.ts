  ...
  try {
    switch (event.type) {
      ...
      case "invoice.paid": {
         ...
         * That is a valid paid invoice, but no customer funds were
      ...
      case "invoice.voided": {
         ...
         * only when it carries the complete provenance written
         * by the MAT wallet payment service.
         *
        ...
        const isWalletSettlement =
          paymentSource === "wallet" &&
          walletReferenceType === "STRIPE_INVOICE" &&
        ...
        if (!isCurrentSubscription) {
          console.warn(
            "STRIPE_WALLET_INVOICE_VOIDED_STALE_SUBSCRIPTION_SKIPPED",
            {
              eventId: event.id,
              invoiceId: invoice.id,
              userId,
...
async function resolveUserIdFromStripeSubscription(
  ...
  /*
   * Recovery path:
   * older Stripe subscriptions may no longer be stored as the
  ...
  /*
   * Never trust an external metadata user ID blindly.
   * It must still correspond to a real local user.
   */
